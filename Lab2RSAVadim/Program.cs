...
namespace Lab2RSAVadim
{
    class RSA
    {
        private byte primP;
        private byte primQ; 
        private ushort produsN;
        private ushort phi; 
      
        private ushort cheiaPublicE;
        private Int32 cheiaPrivataD;
        ...
        private void InitKeyData()
        {
            ...
            this.primP = simple[random.Next(0, simple.Length)];
            this.primQ = simple[random.Next(0, simple.Length)];
            this.produsN = (ushort)(this.primP * this.primQ);
            this.phi = (ushort)((primP - 1) * (primQ - 1));
            List<ushort> possibleE = GetAllPossibleE(this.phi);
            do
            {
                this.cheiaPublicE = possibleE[random.Next(0, possibleE.Count)];
                this.cheiaPrivataD = ExtendedEuclide(this.cheiaPublicE % this.phi, this.phi).u1;
            } while (this.cheiaPrivataD < 0);
        }
        ...
        public string encode(string text)
        {
            ...
            {
                int encryptedValue = ModuloPow(value, this.cheiaPublicE, this.produsN);
...
class Program
    {
        ...
        {
            String text = "rgmaunrfhmqewknutvghmceiwnufhamseiuncfhm";
            RSA rsa = new RSA();
            Console.WriteLine("Encrypted Text este {0}\n", rsa.encode(text));
            String encryptedText = rsa.encode(text);
            String nKey = ""+rsa.GetNKey();
            String dKey = "" + rsa.GetDKey();
            String decryptedText = rsa.decode(encryptedText,nKey,dKey);
            Console.WriteLine("Textul decriptat este {0}\n", decryptedText);
